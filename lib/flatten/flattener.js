...
class Flattener extends OptionHolder {
  ...
  validate(target) {
    if (!isObject(target)) {
      throw new Error(`Invalid flatten target: must be an Object, was: ${this.target}`)
    }
  }
...
function createFlattener(target, opts) {
  return new Flattener(target, opts)
}


function flatten(target, opts) {
  return createFlattener(target, opts).flat()
}
