...
const leafValue = {
  select(opts) {
    ...
    return opts.shallow ? leafValue.shallow : leafValue.expand
  },
...
const $defaults = Object.assign($$defaults, {
  ...
  fetchValue(acc, selectedKey) {
    return acc[selectedKey]
  },
  leafValue,
  ...
  makePointer(obj, path, opts = {}) {
    ...
    const result = path.reduce((acc, $key, index) => {
      ...
      const selectedKey = selectKey ? selectKey(acc, key, keyMatchers) : key
      if (onValueAt) {
        onValueAt(acc, selectedKey)
      }
      const nextValue = fetchValue(acc, selectedKey)
      if (nextValue) {
