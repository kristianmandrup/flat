...
class Unflattener extends Logger {
  ...
  get valueKeys() {
    return Object.keys(this.valueSrc)
  }
  ...
  getKeys() {
    return this.keys || this.valueKeys
  }

  exclude(keys) {
    const excludeList = this.excludeKeys || []
    return keys.filter(key => excludeList.indexOf(key) >= 0)
  }

  keysToInclude() {
    return this.keysFilter(exclude(getKeys()))
  }

  doSteps() {
    const keys = this.keysToInclude()
    this.log('doSteps', {
      keys
    })
