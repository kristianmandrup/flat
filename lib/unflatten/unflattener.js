const { isBuffer, isObject, isNumber, Logger } = require('../util')
...
class Stepper extends Logger {
  constructor(key, unflattener) {
    ...
    this.overwrite = unflattener.overwrite
    this.result = {}
  ...
  novalidOverWrite() {
    return !this.overwrite && !this.isObj && this.notDefined
  }

  step() {
    this.log('key1', this.key1)
    this.log('key2', this.key2, isDefined(this.key2))

    while (isDefined(this.key2)) {
      this.log('changing for', this.key2)
      this.changed = true
      this.isObj = isObject(this.rkey1)
      this.notDefined = !isDefined(this.rkey1)

      // do not write over falsey, non-undefined values if overwrite is false
      // if (this.novalidOverWrite()) {
      //   this.log('novalidOverWrite', this.overwrite, this.isObj, this.notDefined)
      //   return
      // }

      if ((this.overwrite && !isObj) || (!this.overwrite && this.rkey1 == null)) {
    ...
    if (!this.shallow) {
      ...
      let wasChanged = value !== this.rkey1
      this.changed = this.changed || wasChanged 
    } else {
